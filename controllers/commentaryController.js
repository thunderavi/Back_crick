...
// Initialize Google Cloud Text-to-Speech with environment variables
let ttsClient;

try {
  if (process.env.GOOGLE_CLOUD_PRIVATE_KEY && process.env.GOOGLE_CLOUD_CLIENT_EMAIL) {
    console.log('✅ Initializing TTS with environment variables');
    ...
    console.log('✅ Google Cloud TTS initialized with environment credentials');
  } else if (process.env.GOOGLE_APPLICATION_CREDENTIALS) {
    console.log('✅ Initializing TTS with JSON file');
    ttsClient = new textToSpeech.TextToSpeechClient();
    console.log('✅ Google Cloud TTS initialized with JSON file');
  } else {
    console.warn('⚠️ No Google Cloud credentials found. TTS will be disabled.');
    ttsClient = null;
  }
} catch (error) {
...
// ============================================
// SYNTHESIZE SPEECH (TEXT TO AUDIO) - Returns Base64
// ============================================
exports.synthesizeSpeech = async (req, res) => {
  try {
    ...
    if (!ttsClient) {
      console.warn('⚠️ TTS client not available, skipping audio synthesis');
      return res.json({
        success: true,
        audioData: null,
        audioDuration: 0,
...
exports.generateCommentaryWithAudio = async (req, res) => {
  try {
    ...
    // Only attempt TTS if client is available
    if (ttsClient) {
      try {
        const ttsRequest = {
        ...
        // Convert to base64
        audioData = ttsResponse.audioContent.toString('base64');
        audioDuration = estimateAudioDuration(commentaryText);
        
        console.log(`🎵 Audio synthesized as base64 (${audioDuration}s)`);
      } catch (ttsError) {
        console.warn('⚠️ TTS failed, continuing without audio:', ttsError.message);
      }
    } else {
